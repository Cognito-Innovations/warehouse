...
test('test', async ({ browser }) => {
  ...
  await page.getByRole('textbox', { name: 'Reference Tracking' }).fill('111234512357');
  ...
  await expect(page.getByText('111234512357')).toBeVisible({ timeout: 60000 });

  await page.locator('tr:has-text("111234512357")').locator('.MuiIconButton-root').first().click();
  await page.waitForLoadState('networkidle');
  ...
  await visibilityBtn.click({ timeout: 30000 });

  await expect(page).toHaveURL(/.*\/shipments\/S20255933IN/);

  await page.locator('.MuiCircularProgress-root').waitFor({ state: 'detached', timeout: 60000 });

  await expect(page.getByText('Shipment Photos', { exact: true })).toBeVisible({ timeout: 50000 });
