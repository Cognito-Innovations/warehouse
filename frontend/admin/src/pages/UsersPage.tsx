...
import { Box, Typography, TextField, Select, MenuItem, Button, FormControl, InputLabel, CircularProgress, FormHelperText, InputAdornment } from "@mui/material";

import { getCouriers, createUser } from "../services/api.services";
...
const UsersPage: React.FC = () => {
  ...
  const validateField = (name: keyof FormData, value: FormData[typeof name]): string => {
    switch (name) {
      case 'email':
      ...
      case 'role':
        ...
        return '';
      case 'courierId':
        if (!value) return 'Courier Company is required';
        return '';
    }
  };
  ...
  const handleChange = (
    ...
    const value = e.target.value as FormData[typeof name];
  ...
  const handleBlur = (name: keyof FormData) => {
    ...
    const newError = validateField(name, formData[name]);
    setErrors((prev) => ({ ...prev, [name]: newError }));
  };
