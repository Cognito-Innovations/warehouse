  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 h-16">
          ...
          <nav className="flex-1 flex justify-center">
            <ul className="flex space-x-2">
              {navItems.map((item) => (
                <li key={item.name}>
                  <Link href={item.path} className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${ pathname === item.path ? 'bg-purple-600 text-white font-semibold' : 'text-white hover:bg-white hover:bg-opacity-10'}`}>
                    {item.name}
                  </Link>
                </li>
              ))}
      ...
      {pathname === '/assisted-shopping' || pathname ==='/pickup-request' || pathname ==='/pickup-request/create-request' ? (
        <AddressSection />
      ) : pathname === '/assisted-shopping/create-request' || pathname.startsWith('/pickup-request/') ||  pathname.startsWith('/assisted-shopping/') ? null : (
